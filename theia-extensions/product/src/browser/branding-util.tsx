...
/**
 * Command contributed by `theia-ide-updater-ext`. It is referenced by id rather than imported, because that
 * extension is part of the desktop application only and its contribution lives in `electron-browser`.
 */
export const CHECK_FOR_UPDATES_COMMAND = 'electron-theia:check-for-updates';
...
/** A link that triggers an action in the application rather than opening a page. */
function ActionLink(props: { text: string, onClick: () => void }): React.JSX.Element {
    return <a
        role={'button'}
        tabIndex={0}
